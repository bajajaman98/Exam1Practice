...
def problem2b(rect, n, delta, win):
    for k in range(n):
        rectangle = rg.Rectangle(rg.Point(rect.get_upper_left_corner().x - k* delta,rect.get_upper_left_corner().y - k*delta),rg.Point(rect.get_lower_right_corner().x+k*delta,rect.get_lower_right_corner().y+k*delta))
